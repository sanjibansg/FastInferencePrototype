...
'''
for i in graph[0].nodes():
 temp=[x for x in i.attributeNames()]
 attrb={j:i[j] for j in temp}
 print(i.kind())
 print(attrb)
 print('\n')
'''
#print(type(graph[1]['6']))

'''
for k,v in graph[1].items():
 print(v.numpy().shape)
'''
weights=[i[1].numpy() for i in list(model.state_dict().items())]
#print(weights)
...
'''
#gives the attributes/properties of layers
